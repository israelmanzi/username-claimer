	...
	while True:
		...
		try:
			if "Something is wrong." in second_response:
				functions.logtofile("instagram_error_3.txt", second_response)
		...
		try:
			if data['message']['errors'][0] == "This username isn't available. Please try another.":
				print(functions.CRED + "[>] This username is not available.")
				times_attempted += 1
		except:
			pass
		...
		try:	
			if data['status'] == "fail":
				print(functions.CRED + "[>] Instagram returned fail. Username isn't available.")
				times_attempted += 1
		except:
			pass

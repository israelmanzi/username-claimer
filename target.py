...
def start(account, target, delay, proxy):
    ...
    if not email:
        print(functions.CRED +
              "[!] Something went wrong while loading account, make sure the account is still valid: " + account)
        return
    else:
    ...
    while True:
        if not claimed:
            if failed > 20:
                print(functions.CRED +
                      "[>] Closing thread due to 20 failed attempts.")
                return
            else:
                time.sleep(int(delay))
                data = 'first_name=' + firstname + '&email=' + urllib.parse.quote(email) + '&username=' + target + '&phone_number=' + urllib.parse.quote(
                ...
                try:
                    send = requests.post("https://www.instagram.com/accounts/edit/",
                                         data=data, timeout=10, headers=postheaders, proxies=proxy)
                    attempts += 1
                except requests.ConnectionError:
                    failed += 1
                ...
                try:
                    ...
                    if data['status'] == "ok":
                        print(functions.CGREEN + "[>] Successfully claimed username: " + target +
                              " on account: " + account + " [Attempts: " + str(attempts) + "]")
                        claimed.append(target)
                        functions.logtofile(
                            "success_target_" + target + ".txt", account + " > " + target)
...
def target():
    ...
    print(functions.CYAN +
          "[!] This option requires proxy, please fill /files/proxies.txt with fresh proxies.")
    ...
    if question == "y" or question == "Y":
        ...
        if len(check_files) >= 1:
            ...
            for filename in os.listdir("turbo_claim"):
                ...
                if os.path.isfile(f):
                    th = threading.Thread(
                        target=start, args=(f, target, delay, "true"))
                    th.start()
        else:
            print(functions.CRED +
                  "[!] Could not locate any accounts in /turbo_claim/. Fill folder up and try again")
    else:
